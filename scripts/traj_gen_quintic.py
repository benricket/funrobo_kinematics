...
class QuinticPolynomial():
    ...
    def solve(self, q0, qf, qd0, qdf, T):
        ...
        #qddf = np.zeros_like(q0) if qddf is None else np.asarray(qddf,dtype=float)4
    ...
    def generate(self, t0=0, tf=0, nsteps=100):
        ...
        for i in range(self.ndof): # iterate through all DOFs
            ...
            X[i, 0, :] = q      # position
            X[i, 1, :] = qd     # velocityqdd0 = np.zeros_like(q0) if qdd0 is None else np.asarray(qdd0,dtype=float)
...
class Trapezoidal():
    ...
    def generate(self, t0=0, tf=0, nsteps=100):
        ...
        for i in range(self.ndof): # iterate through all DOFs
            #j = np.arange(nsteps)
            #t = (tf - t0)*(j/nsteps) # time at current step
            for j in range(nsteps):
                t = (tf - t0)*(j/nsteps)
                if t < self.tb[i]:
                    q = self.q0[i] + self.alpha[i] * t**2 * 0.5
                    qd = self.alpha[i] * t
                    qdd = self.alpha[i]
                elif t < tf - self.tb[i]:
                    q = 0.5*(self.qf[i] + self.q0[i] - self.V[i]*tf) + self.V[i]*t
                    qd = self.V[i]
                    qdd = 0
                else:
                    q = self.qf[i] - 0.5 * self.alpha[i] * tf**2 + self.alpha[i] * tf * t - 0.5 * self.alpha[i] * t**2
                    qd = self.alpha[i] * (tf - t)
                    qdd = -1 * self.alpha[i]
...
def main():
    ...
    via_points = [[-30, 30, 30, 10], [0, 45, -45, 5], [30, 15, 30, -10], [50, -30, 50, -40]]

    traj.solve(via_points, T=2)
    traj.generate(nsteps_per_segment=50)
